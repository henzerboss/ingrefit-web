...
const photoSchema = z.object({
  ...
  // Validated as base64 so malformed payloads fail here in microseconds
  // instead of after a 35-second Gemini round trip.
  //
  // Whitespace is stripped before validation rather than rejected: some
  // encoders wrap base64 at 76 characters, and an app build already in the
  // wild must not start failing because of a stricter server.
  base64: z
    .string()
    .min(100)
    .max(8_000_000)
    .transform((value) => value.replace(/\s+/g, ''))
    .refine((value) => value.length >= 100 && /^[A-Za-z0-9+/_-]+={0,2}$/.test(value), {
      message: 'Photo data must be base64 encoded.',
    }),
...
export const analyzeRequestSchema = z
  ...
  .superRefine((value, context) => {
    if (value.mode === 'barcode' && (!value.barcode || value.photos))
      context.addIssue({ code: 'custom', message: 'barcode mode requires only a barcode' });
    if (value.mode === 'label') {
      if (!value.photos) {
    ...
    if (value.mode === 'unpackaged' && (!value.photos || value.photos.length !== 1 || value.photos[0]?.kind !== 'food'))
      context.addIssue({ code: 'custom', message: 'unpackaged mode requires one food photo' });
  });
...
export type InstallationRegistration = z.infer<typeof installationRegistrationSchema>;
